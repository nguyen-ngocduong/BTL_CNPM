...
import Specifi from '../components/common/Specifi';
...
export default function Product() {
  ...
  const handleCartAddItem = (variantId: number) => {
    ...
    const currentVariant = cart.find((item) => item.variantId === variantId);
    if (!currentVariant) {
      const newItem = {
        variantId,
        quantity: amount,
          ...
          <Container md css={{ pt: 80 }}>
            <Grid.Container>
                  ...
                  >
                    {product.images.map((i, index) => (
                     ...
                     <div style={{ 
                      ...
                      <Row style={{ padding: '10px' }}>{product.description}</Row>
                  </Swiper>
                </div>
              </Grid>
              {/* <Spacer y={2} /> */}
              <Grid xs={12} sm={6} direction='column'>
                ...
                <Card.Divider />
                <Spacer y={1} />
                <Specifi productId={product?.id.toString()} />
              </Grid>
            </Grid.Container>
            <Spacer y={4} />
          
            <div style={{ marginLeft: '8%' }}> 
              {/* <Text h2 style={{ color: '#7828C8' }}>Mô tả</Text>
              <Row>{product.description}</Row> */}
              {/* <Spacer y={4} /> */}
              <Text h2 style={{ color: '#7828C8' }}>Bình luận</Text>
              <Comment productId = {product?.id.toString()}/>
            </div>
          </Container>
        ) : (
          'Loading...'
